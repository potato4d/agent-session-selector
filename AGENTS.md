...
`C--Users-mail-Documents-repos-cc-session-selector` → `C:\Users\mail\Documents\repos\cc-session-selector`

ファイルシステムを参照して `-` を含むディレクトリ名（`cc-session-selector` など）を正しく復元する（`decodeProjectPath` in `src/server/routes/sessions.ts`）。
